...
const ArticlesListing = () => {
  ...
  const getArticleListing = async () => {
    ...
    let data = await apiCall("POST", "getArticleListing", req_data);
    if (data.code == 1) {
  ...
  const handleChange = () => {
    history.push({ pathname: "/app/Articles/AddEditArticle" });
  };

  const handleDelete = async (item) => {
    const { article_id, name } = item;
    if (
      await confirmBox(
      ...
      let { code } = await apiCall("POST", "deleteArticleById", reqData);

      if (code == 1) {
        getArticleListing();
        displayLog(code, "Article has been successfully deleted");
      } else {
        setLoading(false);
      }
    } else {
  ...
  return (
    <div className="user-management">
      <h1 className="main_text_h1 mb-4">Articles Management</h1>
                ...
                >
                  <span className="has-wrapperss">
                    ...
                    <input
                      ...
                      placeholder="Search Article by name, description, category"
                      type="text"
                      value={searchByName}
                      name="searchbyName"
                        ...
                        list.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td className="list-action">
                              <Tooltip title="Edit Artcile">
                                <Link
                                  to={{
                                    pathname: "Articles/AddEditArticle",
